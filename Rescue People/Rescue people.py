...
spiral_separation = 1 #we want that the radius expand 4 metres in each rotation
angle_step = math.pi / 8 # 45 degrees per step for a rapid spiral expansion
num_steps = 40
fly_height = 3
search_path = []
...
def location_survivor(face_cords):
	...
	if len(survivor_cords) > 1 :
    	for cordenates in survivor_cords:
        	if abs(face_cords[0]-cordenates[0])+abs(face_cords[1] - cordenates[1]) <10:
